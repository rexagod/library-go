...
func ApplyUnstructuredResourceImproved(
	...
	if requiredObjectMetaTyped.ResourceVersion != "" && existingObjectMetaTyped.ResourceVersion != requiredObjectMetaTyped.ResourceVersion {
		err = errors.NewConflict(resourceGVR.GroupResource(), name, fmt.Errorf("update rejected for %s %s because the object has been modified: desired/actual ResourceVersion: %v/%v", existing.GetKind(), existing.GetName(), requiredObjectMetaTyped.ResourceVersion, existingObjectMetaTyped.ResourceVersion))
		return nil, false, err
	}
